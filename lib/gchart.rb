...
class Gchart
  ...
  def set_legend
    ...
    if legend.is_a?(Array)
      "chdl=#{@legend.map{|label| "#{CGI::escape(label.to_str)}"}.join('|')}"
    else
      "chdl=#{legend}"
    end
  ...
  def set_labels
    if labels.is_a?(Array)
      "chl=#{@labels.map{|label| "#{CGI::escape(label.to_str)}"}.join('|')}"
    else
      "chl=#{@labels}"
    end
  end
  ...
  def set_axis_labels
    ...
    labels_arr.map! do |index,labels|
      if labels.is_a?(Array)
        "#{index}:|#{labels.map{|label| "#{CGI::escape(label.to_str)}"}.join('|')}"
      else
        "#{index}:|#{labels}"
      end
    end
